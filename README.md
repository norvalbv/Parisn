...
1. Clone the repo
   ...
   ```
2. Install YARN packages in webapp (access via: `cd webapp` in root folder) folder **and** root folder **and** backend (access via: `cd backend` in root folder (Note, the backend is only temporary for web sockets. This will soon be transferred into AWS)) .
   ```sh
   yarn install
   ```
